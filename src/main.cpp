...
#if defined(NDEBUG)
# error "Lilycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x69d8dc728cc2128fe5b5b4ea8184f7466a7fd3b1e58c1b62a99057780335b2e0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Lilycoin: starting difficulty is 1 / 2^12
...
{
    // Lilycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 20000); // Lilycoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.5 * 24 * 60 * 60; // Lilycoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Lilycoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Lilycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Lilycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbb24258b7a8499b64e2d38fe3c6748e870c1fdf14911ba5693eeeb973d7fb24a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Satoshi Nakamoto...";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042258ca78a3508b25f9ca7a7971ea21dc5e4666bd9bcad839708b65056065dada50cfb9f0436d8894b9827931a27f2b97d32b46b700ab0a4a447b59050a991921") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1619247755;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084854456;

        if (fTestNet)
        {
            block.nTime    = 1619247691;
            block.nNonce   = 385358105;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xc2a1a796e3c50ed73a7454d9874f89b44998a74eb535612aadcdec07e438d602"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xff, 0xd3, 0xf5, 0xda }; // Lilycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LilycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LilycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LilycoinMiner(CWallet *pwallet)
{
    printf("LilycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lilycoin-miner");
    ...
    try { loop {
        ...
        printf("Running LilycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LilycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LilycoinMiner, pwallet));
}
